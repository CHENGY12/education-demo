...
`skill_candidate` 是可选的广义解题技能候选。只有 `auto_promote=true`、该技能有用且新颖并确有跨题复用价值时才输出对象；其他情况必须输出 `null`。
...
- `action` 只能是 `create` 或 `update`；更新时 `related_skill_id` 必须是输入 `solution_skills` 中真实存在的稳定 id，创建时写空字符串。`novelty_rationale` 说明相对已有 skill 的通用增量。
- `useful`、`novel` 和 `generalized` 必须均为 true。该对象只是待审核候选，不因 Teacher 输出而自动成为可信 skill。
