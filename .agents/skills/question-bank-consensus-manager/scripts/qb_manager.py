...
def record_solution_skill(
    ...
    *,
    ...
    """Version and optionally activate a useful, novel shared solution skill."""
    value = _validated_skill_candidate(candidate)
    if (
        value["action"] == "none"
        or not value["useful"]
...
class JobCoordinator:
    ...
    def _run_skill_extraction(
        ...
        try:
            ...
            self._update_skill_job(job_id, "running", 30, "提炼 Agent 正在判断新颖性和复用价值")
            payload, meta = self.pipeline.runner.run(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "status":
            ...
            return 0
        if args.command in {"audit", "expand", "run"}:
            state.ensure()
            pipeline = Pipeline(
                state,
                model=args.model,
