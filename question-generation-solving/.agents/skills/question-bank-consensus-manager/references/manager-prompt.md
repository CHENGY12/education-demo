...
1. 先运行 `doctor`，再对目标目录列表调用 runner 的 `run --dry-run` 和正式 `run`；runner 会完成初始化与扫描。所有命令使用 `$question-bank-consensus-manager` 自带的 `scripts/qb_manager.py`，每个目录用一个重复的 `--target` 传入。
   开始前必须完整读取题库根目录最新版 `README.md`、`validate.py`，以及本 Skill 的 `delivery-issues-v1.md`；三者冲突时采用更严格且不破坏审计/交付哈希的门槛，不得继续沿用旧规范缓存。
2. 对未满足题库配额的节点调用生成 Agent。生成 Agent既负责围绕 `reference.md`/原题生成举一反三题，也必须独立试做；它的解答只作为候选 1。
   根据 cohort 路径确定语言：`hk-*`/含 `hongkong` 的节点使用 `zh-Hant-HK`，题干、选项、解析、提示与面向学生的解题过程全部为香港繁体；其他当前中国大陆节点使用 `zh-Hans-CN`。该字段进入每个 solver/Teacher 的净化请求与审计哈希。
...
6. 每道题默认只做一次自动 3+1。Teacher 判为不一致后不得自动再调用三个 solver：seed/已有题进入 `disagreement`、`invalid` 或 `error` 时都必须保留首轮证据并直接进入“待审查”，不得淘汰或因干净交付过滤而从人工队列消失；生成题保留为“候选不一致”且绝不写入源 `questions.jsonl`。因为配额仍有缺口，下一次 `expand`/`run --mode full` 必须另生成一道不同题面，并把历史淘汰题面作为禁重复清单。替换生成不得接收旧答案、Teacher 答案/解法、诊断 comments、Agent 身份、错误人数或候选原句。
...
8. 先用小范围试运行并执行 `verify`；通过后再扩大范围。不得一开始无上限消耗整个题库。单题 validator 必须覆盖最新版 README 与问题清单中的可确定检查，包括控制/ANSI 字符、数学定界符、Unicode/ASCII 伪数学、花括号、命令反斜杠、`\text{}`/`\mathrm{}`、化学键下标、指数分组、公式中文、标点、结论字母、繁简体与题干/选项分离；不允许 Teacher pass 绕过这些静态门槛。
9. 完成后先执行 `export`，把未解决集合同时原子写到 `错题集.jsonl` 与 Git 可移植的 `review-queue/unresolved.jsonl`，再启动本地审题台。全新 Git 下载后的 `scan`/`serve` 必须能从后者恢复题面、三份 attempt、Teacher review 与题目标注。默认“待审查”包含所有 `status=disagreement|invalid|error` 的 seed 题；其他 disagreement 进入“候选不一致”；生成 invalid/error 可由状态筛选查看；第三个视图为共享解题技能库。旧 portable 记录不得把已 final 的题降级。
...
只有在以下条件同时满足时才宣布这一范围完成：扫描范围固定；每道题为 `final` 或明确进入人工复核；所有未解决 seed 都存在于 portable review queue 且在网页默认“待审查”可见；无静默丢失的任务；`verify` 通过；题库普通 validator 通过；准备交付时 `--prepare-delivery` 与生成目录的 `--delivery` 检查通过；审题网页可打开；最终统计、排除题、答案分布和审计目录已报告。模型输出可追溯重放，但不得宣称随机生成结果可以逐字节复现。
